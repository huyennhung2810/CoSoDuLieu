...
SELECT sv.masv, sv.hoten, pm.sophieu, s.tieude, pm.ngaymuon, pm.ngaytra
FROM phieumuon pm
JOIN sinhvien sv ON pm.masv = sv.masv
JOIN chitetphieu ctp ON pm.sophieu = ctp.sophieu
...
-- 6.4
SELECT COUNT(pm.masv) AS LuotSinhVienMuonSach
FROM PhieuMuon pm
WHERE pm.trangThai = N'Đã trả' OR pm.trangThai = N'Chưa trả'
--6.6 Hiển thị tất cả các quyển sách có tiêu đề chứa từ
--khoá “SQL”.
SELECT tieude, masach, giatien
FROM sach
WHERE tieude LIKE '%tieude03%';
...
--xếp thứ tự theo ngày mượn sách.
SELECT sv.masv, sv.hoten, pm.sophieu, s.tieude, pm.ngaymuon, pm.ngaytra
FROM phieumuon pm
JOIN sinhvien sv ON pm.masv = sv.masv
JOIN chitetphieu ctp ON pm.sophieu = ctp.sophieu
...
WHERE ngaysinh = (
	...
	WHERE (ngaymuon >= '2023-8-1' and ngaymuon <= '2023-8-31')
)